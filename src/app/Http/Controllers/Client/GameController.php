...
class GameController extends Controller
{
    ...
    public function submitAnswer(Request $request, Game $game, GameSession $session)
    {
        // Verify session is active and belongs to user
        if (!$session->isActive() || $session->client_id !== Auth::guard('client')->id()) {
        ...
        // Get the index of the submitted answer in the choices array
        $submittedIndex = array_search($request->answer, $question->choices);
        // Compare the submitted index (0-based) with correct_choice (1-based)
        $isCorrect = ($submittedIndex + 1) === $question->correct_choice;
        ...
        // Record the answer with correct points
        ...
        if (!$session->hasLivesRemaining()) {
            $session->endGame('lives_exhausted');
            return response()->json([
                'correct' => $isCorrect,
                'gameEnded' => true,
                'reason' => 'lives_exhausted',
                'redirect' => route('games.result', ['game' => $game->slug, 'session' => $session->id])
            ]);
        }

        // Get next question
        $nextQuestion = $this->getNextQuestion($session, $question->id);
        ...
        return response()->json([
            ...
            'score' => $session->score,
            'timeRemaining' => $session->time_remaining,
            'pointsEarned' => $points,  // Add points earned to response
        ]);
    }
